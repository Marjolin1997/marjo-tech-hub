...
class CategoryController extends Controller
{
    ...
    private function assertOwner(Request $request, Category $category): void
    {
        abort_unless($category->user_id === $request->user()->id, 404);
    }

    private function assertOwnedParent(Request $request, ?int $parentId): void
    {
        if ($parentId === null) return;
        abort_unless($request->user()->categories()->whereKey($parentId)->exists(), 422, 'The selected parent category is invalid.');
    }

    private function isDescendant(Category $category, int $candidateId): bool
    {
        $cursor = Category::find($candidateId);
        while ($cursor) {
            if ($cursor->parent_id === $category->id) return true;
            $cursor = $cursor->parent;
        }
        return false;
    }

    private function uniqueSlug(Request $request, string $name, ?int $parentId, ?int $ignoreId = null): string
    {
