...
class EntryController extends Controller
{
    public function index(Request $request)
    {
        ...
        $user = $request->user();
        $query = $user->entries()->with(['category', 'tags'])->withExists([
            'favorites as is_favorite' => fn ($q) => $q->where('user_id', $user->id),
        ]);
    ...
    public function store(EntryRequest $request): JsonResponse
    {
        ...
        $this->assertOwnedTags($request, $tagIds);
        $payload = Arr::except($data, ['tag_ids']);
        $payload['slug'] = $this->uniqueSlug($request, $data['title']);
        $entry = $request->user()->entries()->create($payload);
    ...
    public function update(EntryRequest $request, Entry $entry): EntryResource
    {
        ...
        $this->assertOwnedTags($request, $tagIds);
        $payload = Arr::except($data, ['tag_ids']);
        $payload['slug'] = $this->uniqueSlug($request, $data['title'], $entry->id);
    ...
    private function hydrate(Request $request, Entry $entry): Entry
    {
        $entry->load(['category', 'tags']);
        $entry->loadExists(['favorites as is_favorite' => fn ($q) => $q->where('user_id', $request->user()->id)]);
        return $entry;
    }
    ...
    {
        $base = Str::slug($title) ?: 'entry'; $slug = $base; $suffix = 2;
        while ($request->user()->entries()->where('slug', $slug)->when($ignoreId, fn ($q) => $q->whereKeyNot($ignoreId))->exists()) $slug = $base.'-'.$suffix++;
        return $slug;
    }
}
