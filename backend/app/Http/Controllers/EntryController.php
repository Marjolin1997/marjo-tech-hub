...
class EntryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $validated = $request->validate(['category_id'=>['nullable','integer'],'type'=>['nullable','in:command,snippet,note'],'tag_id'=>['nullable','integer'],'favorite'=>['nullable','boolean'],'q'=>['nullable','string','max:120'],'per_page'=>['nullable','integer','min:1','max:50']]);
        $user=$request->user(); $query=$user->entries()->with(['category','tags'])->withExists(['favorites as is_favorite'=>fn($q)=>$q->where('user_id',$user->id)]);
        if(!empty($validated['category_id']))$query->where('category_id',$validated['category_id']); if(!empty($validated['type']))$query->where('type',$validated['type']);
        if(!empty($validated['tag_id']))$query->whereHas('tags',fn($q)=>$q->where('tags.id',$validated['tag_id'])->where('tags.user_id',$user->id)); if(($validated['favorite']??false)===true)$query->whereHas('favorites',fn($q)=>$q->where('user_id',$user->id));
    ...
    public function show(Request $request, Entry $entry): EntryResource { $this->requirePermission($request,'entries.view');$this->assertOwner($request,$entry);return new EntryResource($this->hydrate($request,$entry)); }
