...
class DocumentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = $request->validate(['q'=>['nullable','string','max:120'],'category_id'=>['nullable','integer'],'tag_id'=>['nullable','integer'],'is_sensitive'=>['nullable','boolean'],'extension'=>['nullable','string','max:12'],'date_from'=>['nullable','date_format:Y-m-d'],'date_to'=>['nullable','date_format:Y-m-d','after_or_equal:date_from'],'per_page'=>['nullable','integer','min:1','max:50']]);
        $user=$request->user(); $query=$user->documents()->with(['category','tags']);
        ...
        if(!empty($data['date_from']))$query->whereDate('created_at','>=',$data['date_from']); if(!empty($data['date_to']))$query->whereDate('created_at','<=',$data['date_to']);
