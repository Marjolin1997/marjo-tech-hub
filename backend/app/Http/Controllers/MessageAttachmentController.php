...
class MessageAttachmentController extends Controller
{
    ...
    public function store(Request $request, Conversation $conversation): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'kind' => ['required', 'in:file,image,voice'],
        ...
        $storedName = Str::uuid().'.'.preg_replace('/[^a-z0-9]+/', '', $extension);
        $relativePath = 'messages/'.$conversation->id.'/'.$storedName;
        ...
        try {
            ...
            $message = DB::transaction(function () use ($conversation, $request, $body, $disk, $relativePath, $file, $mime, $kind, $checksum, $validated): Message {
                ...
                $message->attachments()->create([
                    ...
                    'duration_ms' => $kind === 'voice' ? ($validated['duration_ms'] ?? null) : null,
    ...
    public function download(Request $request, MessageAttachment $attachment): StreamedResponse
    {
        ...
        $this->participantOrFail($attachment->message->conversation, $request->user()->id);
    ...
    public function media(Request $request, MessageAttachment $attachment): StreamedResponse
    {
        ...
        $attachment->loadMissing('message');
        $this->participantOrFail($attachment->message->conversation, $request->user()->id);
        abort_unless(in_array($attachment->kind, ['image', 'voice'], true), 404);
        abort_unless(Storage::disk($attachment->disk)->exists($attachment->path), 404);

        return Storage::disk($attachment->disk)->response($attachment->path, null, [
            'Content-Type' => $attachment->mime_type,
            'X-Content-Type-Options' => 'nosniff',
    ...
    private function participantOrFail(Conversation $conversation, int $userId): void
    {
        abort_unless($conversation->participantStates()->where('user_id', $userId)->exists(), 404);
    }

    private function validateKind(string $kind, string $mime, int $size): void
    {
        $allowedFiles = ['application/pdf','text/plain','text/markdown','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','application/zip'];
        ...
        if (! in_array($mime, $allowed, true) || ($kind === 'voice' && $size > self::MAX_VOICE_KB * 1024)) {
            throw ValidationException::withMessages(['file' => ['This file type or size is not allowed for the selected attachment type.']]);
        }
    }
    ...
    private function messagePayload(Message $message): array
    {
        return [
            ...
            'sender' => $message->sender?->only(['id','name','first_name','last_name','username','job_title']),
            'attachments' => $message->attachments->map(fn (MessageAttachment $a) => [
                'id' => $a->id, 'kind' => $a->kind, 'name' => $a->original_name, 'mime_type' => $a->mime_type,
                'size_bytes' => $a->size_bytes, 'duration_ms' => $a->duration_ms,
                'download_url' => '/api/messaging/attachments/'.$a->id.'/download',
                'media_url' => in_array($a->kind, ['image','voice'], true) ? '/api/messaging/attachments/'.$a->id.'/media' : null,
            ])->values(),
