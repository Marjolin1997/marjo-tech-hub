...
class ServiceController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($q = trim((string) ($validated['q'] ?? ''))) {
            ...
            $query->where(fn ($builder) => $builder
                ...
                ->orWhere('owner_label', 'like', "%{$escaped}%"));
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $data['slug'] = $this->uniqueSlug($request, $data['name']);
    ...
    {
        $service = $this->owned($request, $service);
    ...
    {
        $this->owned($request, $service)->delete();
    ...
    private function validatedPayload(Request $request): array
    {
        $data = $request->validate([
            ...
            'lifecycle' => ['required', Rule::in(self::LIFECYCLES)],
            'description' => ['nullable', 'string', 'max:2000'],
            'repository_url' => ['nullable', 'url:http,https', 'max:2048'],
            'production_url' => ['nullable', 'url:http,https', 'max:2048'],
            'api_base_url' => ['nullable', 'url:http,https', 'max:2048'],
            'technologies' => ['nullable', 'array', 'max:30'],
            'technologies.*' => ['string', 'max:60', 'distinct'],
            'owner_label' => ['nullable', 'string', 'max:120'],
        ]);
    ...
    private function owned(Request $request, Service $service): Service
    {
    ...
    private function uniqueSlug(Request $request, string $name, ?int $ignoreId = null): string
    {
        ...
        $slug = $base; $suffix = 2;
        while ($request->user()->services()->where('slug', $slug)->when($ignoreId, fn ($q) => $q->whereKeyNot($ignoreId))->exists()) {
