...
export async function login(credentials){await http.get('/sanctum/csrf-cookie');const{data}=await http.post('/api/auth/login',credentials);return data;}
export async function verifyTwoFactor(payload){const{data}=await http.post('/api/auth/verify-2fa',payload);return data;}
