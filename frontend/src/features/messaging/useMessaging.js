...
import { archiveConversation, createConversation, getConversations, getMessages, getMessagingUsers, markConversationRead, sendMessage, uploadMessageAttachment } from './api.js';
...
const refreshMessaging=(qc,conversationId)=>{qc.invalidateQueries({queryKey:['messaging','messages',conversationId]});qc.invalidateQueries({queryKey:CONVERSATIONS_KEY});};
...
export function useSendMessage(){const qc=useQueryClient();return useMutation({mutationFn:sendMessage,onSuccess:(_,v)=>refreshMessaging(qc,v.conversationId)});}
export function useUploadMessageAttachment(){const qc=useQueryClient();return useMutation({mutationFn:uploadMessageAttachment,onSuccess:(_,v)=>refreshMessaging(qc,v.conversationId)});}
