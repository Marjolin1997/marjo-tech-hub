...
import { useCreateEntry, useTags, useUpdateEntry } from './useLibrary.js';
...
export default function EntryModal({ open, onClose, categories, entry = null, defaultCategoryId = null }) {
  ...
  return <Modal title={entry ? 'Edit entry' : 'Add knowledge entry'} open={open} onCancel={onClose} onOk={submit} okText={entry ? 'Save changes' : 'Create entry'} confirmLoading={mutation.isPending} width={760} destroyOnHidden>
    ...
    <Form form={form} layout="vertical" requiredMark="optional" className="entry-form">
      ...
      <div className="form-grid">
        ...
        <Form.Item name="category_id" label="Category"><Select allowClear placeholder="Uncategorized" options={flattenCategories(categories)} /></Form.Item>
      </div>
      <div className="form-grid">
        <Form.Item name="language" label="Language / shell"><Input placeholder="bash, php, javascript, sql..." maxLength={50} /></Form.Item>
        <Form.Item name="tag_ids" label="Tags"><Select mode="multiple" allowClear maxTagCount="responsive" placeholder="Choose tags" options={tags.map((tag) => ({ value:tag.id, label:tag.name }))} /></Form.Item>
      </div>
      ...
      <Form.Item name="content" label="Content" rules={[{ required:true, message:'Add the command, snippet or note content.' }]}><TextArea className="code-editor" rows={10} placeholder="Paste or write reusable knowledge here..." /></Form.Item>
